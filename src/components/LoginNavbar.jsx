...
export default function LoginNavbar() {
    ...
    return isLoginPage || isRegisterPage ? <></> : <nav className="w-full z-50 bg-white shadow-md">
        {/* <!-- container --> */}
        <div className="container mx-auto flex flex-wrap justify-between items-center md:px-4 px-1 py-3 lg:space-x-4">
            <div className='flex md:gap-7'>
                {/* <!-- brand --> */}
                <Link to={`/`}><img className='hidden lg:inline-block' src="/images/logo.svg" alt="Logo" /></Link>
                <div class="relative">
                    <input name="search" class="px-5 py-1 bg-gray-200 lg:w-96 h-full rounded-lg focus:outline-none" placeholder="Cari disini .." />
            ...
            <div className={`font-medium w-full mt-2 lg:inline-flex lg:w-auto lg:mt-0 ${isMobileNavOpen ? 'flex' : 'hidden'}`}>
                <ul className="flex flex-col w-full space-y-2 lg:w-auto lg:flex-row lg:gap-5 lg:items-center lg:space-y-0 lg:space-x-2">
                    <li><button className='flex items-center gap-2 bg-primaryPurple rounded-lg h-10 py-2 px-3 text-white text-sm'>
