  ...
  return (
    <div className="relative" ref={wrapperRef}>
      <button onClick={handleSetDropdownOpen}>
        {(sellerNotification !== 'null' || buyerNotification !== 'null') ? dotBell : bell }
      </button>
      ...
      <div className={`absolute -right-7 p-5 mt-1 bg-white rounded-xl border shadow-xl 
        ...
        <ul className="lg:w-96 w-72">
          ...
          {Object.keys(sellerNotification).length < 1 && Object.keys(buyerNotification).length < 1 && (
            <h1 className="text-center">Currently Empty</h1>
          )}

          {/* SELLER NOTIFICATION */}
          {(Object.keys(sellerNotification).length > 0 && (sellerNotification.penawaran_masuk.length > 0 || sellerNotification.produk_terjual.length > 0))
          ...
          {/* BUYER NOTIFICATION */}
          {(Object.keys(buyerNotification).length > 0 && (buyerNotification.penawaran_diterbitkan.length > 0 || buyerNotification.penawaran_sudah_diterima.length > 0 || buyerNotification.produk_dibeli.length > 0))
          && buyerNotification.penawaran_diterbitkan.concat(buyerNotification.penawaran_sudah_diterima, buyerNotification.produk_dibeli.length).map((item) => ( 
              <NotificationItem
                productName={item.product.nama}
                productPrice={item.product.harga}
                productBargainedPrice={item.harga_tawar}
