...
export default function HomepageProductList() {
   ...
   useEffect(() => {
      category === 'Semua' ? setFilteredDataDummy(dataDummy)
         : setFilteredDataDummy(dataDummy.filter(item => item.category === category))
   }, [category])
   ...
   return <><section className="container mx-auto px-2 my-10">
      ...
      <section className="container mx-auto px-2 mb-28 mt-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 justify-between gap-3">
         {filteredDataDummy.length < 1 ? <EmptyProductNotification isOnGrid={true} />
            : filteredDataDummy.map((item, index) => <ProductItem key={index} name={item.name} category={item.category} price={item.price} />)}
      </section></>
}