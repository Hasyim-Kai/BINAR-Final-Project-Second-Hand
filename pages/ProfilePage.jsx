...
export default function ProfilePage() {
    ...
    function handleUploadImages(event) { setSelectedFile(event.target.files[0]) }
    ...
    return <div className="min-h-screen max-w-4xl mx-auto relative flex flex-col items-center gap-4 pt-10">
        ...
        <label htmlFor="profilePhoto" className="cursor-pointer">
            <div className="overflow-hidden w-28 h-28 rounded-xl hover:scale-110 transition-all duration-300">
                <img className={`object-cover h-full w-full`} src={preview} alt="cat" />
            </div>
            <input onChange={handleUploadImages} className="hidden" type="file" name="profilePhoto" accept="image/*" id="profilePhoto" />
        </label>
        ...
        <form className="w-3/5 mt-10 mb-24">
            ...
            <div className="mb-5">
                <label>Kota</label><br />
                <input className={inputStyle} type="text" name="kota" placeholder="Kota Anda" required />
            </div>

            <div className="mb-5">
                <label>Alamat</label><br />
                <input className={inputStyle} type="text" name="alamat" placeholder="Alamat Anda" required />
            </div>

            <div className="mb-8">
                <label>No Handphone</label><br />
                <input className={inputStyle} type="number" name="nohp" placeholder="Contoh: 08968888123" pattern="{6,}" required />
            </div>
